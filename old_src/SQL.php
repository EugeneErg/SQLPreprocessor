...
final class SQL
{
    ...
    private function fromStructureValidation(\StdClass $value, Raw $query = null)
    {
        $value->query = call_user_func_array(is_null($query) ? [Raw::class, 'create'] : [$query, 'addChild'], $value->function->getValues());
    }

    private function deleteStructureValidation(\StdClass $value, Raw $query = null)
    {
        $value->query = call_user_func_array(is_null($query) ? [Raw::class, 'createDeleted'] : [$query, 'addDeleted'], $value->function->getValues());
    }

    private function selectStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
    ...
    private function varStructureValidation(\StdClass $value, Raw $query)
    {
    ...
    private function varStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
    }

    private function fromStructureParentValidation(\StdClass $value, \StdClass $structure = null, Raw $query)
    {
    ...
    private function deleteStructureParentValidation(\StdClass $value, \StdClass $structure = null, Raw $query)
    {
        $this->fromStructureParentValidation($value, $structure, $query);
    }

    private function orderbyStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
        $args = $value->function->getArgs();
    ...
    private function groupbyStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
    ...
    private function updateStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
        $args = $value->function->getArgs();
        if (!count($args)) {
            throw new \Exception('неправильный тип аргумента');
    ...
    private function intoStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
        $args = $value->function->getArgs();
        $query->addIntoTable($args[0]->getValue());
    }

    private function insertStructureParentValidation(\StdClass $value, \StdClass $structure, Raw $query)
    {
        $args = $value->function->getArgs();
    ...
    final private function getQueryInfo(Self $fQuery, Raw $query)
    {
        /*
    ...
    private function getUseArgumentArgs(Argument $argument, Raw $query)
    {
        $level = 0;
    ...
    private function getUseFunctionArgs(SQLFunction $function, Raw $query)
    {
    ...
    private function getUseFunctions(array $functions, Raw $query)
    {
    ...
    private function getUseChilds(array $childs, Raw $query)
    {
    ...
    private function getFields(\StdClass $structure, Raw $query)
    {
    ...
    private function parseTreeFunctions(\StdClass $structure, array $keys, Raw $query)
    {
        if (isset($structure->query)) {
            $query = $structure->query;
        }
